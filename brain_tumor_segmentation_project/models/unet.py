...
class ModalityGate(nn.Module):
    """Lightweight modality weighting block for 4 MRI sequences."""
...
class GatedUNet(nn.Module):
    def __init__(self, in_channels: int = 4, out_channels: int = 1, base_channels: int = 32) -> None:
        super().__init__()
        self.gate = ModalityGate(in_channels)
        self.unet = UNet(in_channels=in_channels, out_channels=out_channels, base_channels=base_channels)

    def forward(self, x: torch.Tensor) -> dict[str, torch.Tensor]:
        gated, weights = self.gate(x)
        output = self.unet(gated)
        output["gate_weights"] = weights
        return output
